...
class Scanner:
    ...
    def _extract_sections(self) -> None:
        ...
        while pos < length:
            ...
            if pos < length and text[pos] == "{" and self._looks_like_trailer(text, pos):
                break
    ...
    def _looks_like_trailer(self, text: str, pos: int) -> bool:
        ...
        Un trailer aparece cuando el bloque actual consume todo el
        contenido restante, dejando solo espacios o comentarios.

        Esto permite aceptar especificaciones con placeholders como
        {trailer} al final del archivo sin confundirlos con una
        referencia {ident} dentro de una regla válida.
        """
        try:
            ...
            return False
        rest = text[end + 1 :]
        rest = _strip_ocaml_comments_preserving_layout(rest)
        if not rest.strip():
            return True
        return False


def _strip_ocaml_comments_preserving_layout(text: str) -> str:
    """Reemplaza comentarios OCaml por espacios, preservando saltos de línea."""
    result: list[str] = []
    i = 0
    depth = 0
    ...
    while i < len(text):
        ...
        if i + 1 < len(text) and text[i] == '*' and text[i + 1] == ')' and depth > 0:
            depth -= 1
            result.extend("  ")
            i += 2
            continue
